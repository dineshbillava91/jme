<?php 
	...
	if(!empty($tvseries)){
		foreach($tvseries as $tvseriesRelate){
			...
			foreach($tvseriesRelate['VideoTvseries'] as $singleVideoDetails){
				...
				//CHECK: only add the videos in the response array having the status '0'-non deleted.
	...
	if($flag){
		$arrGetRelatedSeriesList = array('error'=>'0','errorMessage'=>'success','tvseriesVideosCount'=>$tvseriesCount,'tvseriesList'=>$arr);
	}else{
		$arrGetRelatedSeriesList = array('error'=>'1','errorMessage'=>'error','tvseriesVideosCount'=>0,'tvseriesList'=>$arr);
	}
